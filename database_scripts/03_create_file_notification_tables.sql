...
CREATE INDEX IF NOT EXISTS idx_files_search ON files USING gin(to_tsvector('chinese', file_name || ' ' || COALESCE(original_name, '') || ' ' || COALESCE(description, '') || ' ' || array_to_string(tags, ' ')));
