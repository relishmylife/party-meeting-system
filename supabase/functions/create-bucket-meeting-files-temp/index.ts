...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "image/*", "text/*"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for meeting-files" ON storage.objects FOR SELECT USING (bucket_id = 'meeting-files');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for meeting-files" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'meeting-files');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for meeting-files" ON storage.objects FOR UPDATE USING (bucket_id = 'meeting-files');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for meeting-files" ON storage.objects FOR DELETE USING (bucket_id = 'meeting-files');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'meeting-files',
        public: true,
        allowed_mime_types: ["application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "image/*", "text/*"],
