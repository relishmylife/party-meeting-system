-- Migration: create_file_notification_tables_fixed2
-- Created at: 1764405032

-- 党组织生活会议管理系统 - 文件和通知表（修复版2）
...
-- 使用简单搜索但简化标签部分
CREATE INDEX IF NOT EXISTS idx_files_search ON files USING gin(to_tsvector('simple', file_name || ' ' || COALESCE(original_name, '') || ' ' || COALESCE(description, '')));
