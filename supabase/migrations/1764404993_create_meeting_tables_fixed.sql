-- Migration: create_meeting_tables_fixed
-- Created at: 1764404993

-- 党组织生活会议管理系统 - 会议相关表（修复版）
...
-- 使用简单搜索而不是中文搜索
CREATE INDEX IF NOT EXISTS idx_meetings_search ON meetings USING gin(to_tsvector('simple', title || ' ' || COALESCE(content, '') || ' ' || COALESCE(location, '')));
...
-- 使用简单搜索而不是中文搜索
CREATE INDEX IF NOT EXISTS idx_meeting_records_search ON meeting_records USING gin(to_tsvector('simple', title || ' ' || content));
